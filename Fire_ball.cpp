...
void Fire_ball::set_skill(Being * being) {
	damage = 0.4*being->r_inteligence() + 0.5*being->r_weapon()->r_damage();
	effect = being->r_weapon()->r_effect();
	m_damage = 1.2*being->r_weapon()->r_m_damage() + 1.2*being->r_inteligence();
}

bool Fire_ball::use_skill(Being * being, Being * target) {
	set_skill(being);
	if (being->r_stamina() > cost) {
		...
		if (cause_m_damage - m_defence < 2)cause_m_damage = 1;
			...
			target->get_all_effect()->push_back(std::make_pair(effect, 2));
		}
