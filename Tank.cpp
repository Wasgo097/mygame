...
void Tank::save_to_file() const {
	string path = "tank";
	std::ofstream str;
	str.open("save/" + path + "/tank.bin", std::ios::binary | std::ios::out);
...
void Tank::load_from_file() {
	string path = "tank";
	std::ifstream main;
	main.open("save/" + path + "/tank.bin", std::ios::binary | std::ios::in);
	if (!main.good()) {
