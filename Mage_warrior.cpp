...
void Mage_warrior::save_to_file() const {
	string path = "m_warrior";
	std::ofstream str;
	str.open("save/" + path + "/m_warrior.bin", std::ios::binary | std::ios::out);
...
void Mage_warrior::load_from_file() {
	string path = "m_warrior";
	std::ifstream main;
	main.open("save/" + path + "/m_warrior.bin", std::ios::binary | std::ios::in);
	if (!main.good()) {
