...
void Flame_shot::set_skill(Being * being) {
	damage = 1.5*(being->r_a_dexterity() + being->r_b_dexterity()) + 0.3*(being->r_a_strenght() + being->r_b_strenght()) + 0.6*being->r_weapon()->r_damage();
	if (is_type<m_Bow>(being->r_weapon())) {
		effect = being->r_weapon()->r_effect();
		m_damage = 0.4*being->r_weapon()->r_m_damage() *(0.5*(being->r_a_dexterity() + being->r_b_dexterity()));
	}
	else {
		effect = Effect::none;
		m_damage = 0;
...
bool Flame_shot::use_skill(Being * being, Being * target) {
	set_skill(being);
