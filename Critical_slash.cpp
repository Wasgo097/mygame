...
void Critical_slash::set_skill(Being * being) {
	damage = 1.3*being->r_strenght() + 0.2*being->r_dexterity() + 0.5*being->r_weapon()->r_damage();
	if (is_type<m_Sword>(being->r_weapon())) {
...
bool Critical_slash::use_skill(Being * being, Being * target) {
	set_skill(being);
