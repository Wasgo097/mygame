...
void Archer::save_to_file() const{
	string path = "archer";
	std::ofstream str;
	str.open("save/" + path + "/archer.bin", std::ios::binary | std::ios::out);
...
void Archer::load_from_file(){
	string path = "archer";
	std::ifstream main;
	main.open("save/" + path + "/archer.bin", std::ios::binary | std::ios::in);
	if (!main.good()) {
	...
	weap_type.open("save/" + path + "/bow_type.bin", std::ios::binary | std::ios::in);
	if (!weap_type.good()) {
		std::cout << "Loading error";
		exit(-1);
	}
