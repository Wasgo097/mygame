...
// Inherited via Skill
 ...
 void Poison_shot::set_skill(Being * being) {
	damage = 1.1*(being->r_a_dexterity() + being->r_b_dexterity()) + 0.2*(being->r_a_strenght() + being->r_b_strenght()) + 0.5*being->r_weapon()->r_damage();
 ...
 bool Poison_shot::use_skill(Being * being, Being * target) {
	set_skill(being);
